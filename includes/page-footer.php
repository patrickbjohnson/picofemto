	  <!-- Footer -->
	  <footer id="footer" class="footer">
	    <div class="container">
	    	<div class="row">
	    		<div class="col-md-3 col-xs-12">
						<a class="footer-brand pic" href="<?php echo BASE; ?>">
						  <img src="../img/pico-logo.svg" alt="">
						</a>
	    		</div>
	    		<div class="col-md-3 col-xs-12 col-md-offset-1">
	    			<a href="<?php echo BASE; ?>/about" class="footer-section-title">About Us</a>
	    			<p>We create easy-to-use, cloud-based software that helps medical professionals analyze raw primary medical data at the point of diagnosis or evaluation.</p>
	    		</div>
	    		<div class="col-md-3 col-xs-12 footer-menu">
	    			<a href="#" class="footer-section-title">Menu</a>
	    			<div class="row">
	    				<ul>
	    					<li>
	    						<a href="<?php echo BASE; ?>/about">About Us</a>
	    						<ul class="footer-menu-sub-nav">
	    							<li><a href="<?php echo BASE; ?>/team">Team</a></li>
	    							<li><a href="<?php echo BASE; ?>/join">Join</a></li>
	    						</ul>
	    					</li>
	    					<li><a href="<?php echo BASE; ?>/technology">Technology</a></li>
	    					<li><a href="<?php echo BASE; ?>/resources">Resources</a></li>
	    				</ul>
	    				<ul>
	    					<li>
	    						<a href="#">Products</a>
	    						<ul class="footer-menu-sub-nav">
	    							<li><a href="<?php echo BASE; ?>/neurology">Neurology</a></li>
	    							<li><a href="<?php echo BASE; ?>/imaging">Imaging</a></li>
	    							<li><a href="<?php echo BASE; ?>/genomics">Genomics</a></li>
	    						</ul>
	    					</li>
	    					<li><a href="<?php echo BASE; ?>/contact">Contact Us</a></li>
	    				</ul>
	    			</div>
	    			
	    		</div>
	    		
	    	</div>
	    	<div class="row">
	    		<div class="footer-company center">
	    			<ul>
	    				<li>2014 &copy; Picofemto</li>
	    				<li><a href="<?php echo BASE; ?>/terms">Terms &amp; Conditions</a></li>
	    				<li><a href="<?php echo BASE; ?>/privacy">Privacy Policy</a></li>
	    			</ul>
	    		</div>
	    	</div>
	    </div>
	  </footer>
	  <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.0/jquery.min.js"></script>
	  <script src="https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false"></script>
    <script src="https://netdna.bootstrapcdn.com/bootstrap/3.1.1/js/bootstrap.min.js"></script>
	  <script src="../js/bootstrap-prettyfile.js"></script>
	  <script src="../js/main.js"></script>
		<script type='text/javascript'>
	(function (d, t) {
	  var bh = d.createElement(t), s = d.getElementsByTagName(t)[0];
	  bh.type = 'text/javascript';
	  bh.src = '//www.bugherd.com/sidebarv2.js?apikey=7mdpruatexpgtpyajfkvdq';
	  s.parentNode.insertBefore(bh, s);
	  })(document, 'script');
	</script> 
	</body>
</html>